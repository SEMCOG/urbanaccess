...
def gtfsfeed_to_df(gtfsfeed_path=None, validation=False, verbose=True,
    ...
    for index, folder in enumerate(folderlist):
        ...
        merged_routes_df = pd.concat([merged_routes_df, routes_df],
                                                   ignore_index=True)
        merged_trips_df = pd.concat([merged_trips_df,trips_df], ignore_index=True)
        merged_stop_times_df = pd.concat([merged_stop_times_df,stop_times_df],
                                                           ignore_index=True)
        merged_calendar_df = pd.concat([merged_calendar_df, calendar_df],
                                                       ignore_index=True)
